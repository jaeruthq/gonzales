...
class PropietarioUpdateRequest extends FormRequest
{
    ...
    {
        return [
            'nom' => 'min:3|max:20|regex:/^[\pL\s]+$/u|required:propietarios,nom,'.$this->propietario->id,
            'apep' => 'min:3|max:20|alpha:propietarios,apep,'.$this->propietario->id,
            'apem' => 'nullable|max:20|alpha:propietarios,apem,'.$this->propietario->id,
            'ci' => 'min:7|max:10|unique:propietarios,ci,'.$this->propietario->id,
           // 'correo' => 'email:propietarios,correo',
           'fono' => 'nullable|max:8:propietarios,fono,'.$this->propietario->id,
            'cel' => 'min:8|max:8:propietarios,cel,'.$this->propietario->id,
        ];
    }
